...
const TIMEFRAMES = [["1week", "W1", 120], ["1day", "D1", 180], ["4h", "H4", 180], ["1h", "H1", 240], ["15min", "M15", 240], ["5min", "M5", 240]] as const;

type Quote = { price?: string; message?: string };
function json(body: unknown, status = 200) { return Response.json(body, { status, headers: { "Cache-Control": "no-store, no-cache, max-age=0, must-revalidate" } }); }

async function series(apiKey: string, interval: string, outputsize: number): Promise<Candle[]> {
  const url = new URL("https://api.twelvedata.com/time_series");
  url.searchParams.set("symbol", SYMBOL); url.searchParams.set("interval", interval); url.searchParams.set("outputsize", String(outputsize)); url.searchParams.set("format", "JSON"); url.searchParams.set("apikey", apiKey);
  ...
  return [...data.values].reverse().map((c) => ({ time: c.datetime, open: Number(c.open), high: Number(c.high), low: Number(c.low), close: Number(c.close) })).filter((c) => [c.open, c.high, c.low, c.close].every(Number.isFinite));
}
...
export async function GET() {
  ...
  try {
    ...
    const quoteResponse = await fetch(quoteUrl, { cache: "no-store", headers: { Accept: "application/json" } });
    const quoteData = await quoteResponse.json() as Quote;
    const quote = Number(quoteData.price);
    if (!quoteResponse.ok || !Number.isFinite(quote)) throw new Error(quoteData.message || "Live XAU/USD quote unavailable");

    const results = await Promise.all(TIMEFRAMES.map(async ([interval, label, size]) => { const candles = await series(apiKey, interval, size); return { interval, label, candles, analysis: analyzeTimeframe(label, candles) }; }));
    ...
    const reasons: string[] = [direction ? `Weekly/Daily bias is ${direction}.` : "Weekly and Daily do not provide aligned directional bias."];
    if (!htfStructureAligned) reasons.push("No aligned H4/H1 structure shift is verified yet."); else reasons.push("H4/H1 structure is compatible with the macro direction.");
    ...
    if (anchor) {
      ...
      if (!inPoi) reasons.push("Price is outside the 0.71 POI zone."); else reasons.push("Price is inside the 0.71–0.95 POI/stop reference zone.");
    ...
    return json({ live: true, configured: true, symbol: SYMBOL, provider: "Twelve Data", current, status, direction, reasons, fibonacci: anchor, liquidity: sweep, timeframes: results.map((r) => ({ ...r.analysis, interval: r.interval })), execution: { m15: m15.analysis, m5: m5.analysis }, updatedAt: new Date().toISOString(), quoteSource: "Twelve Data /price", rules: { poi: 0.71, stop: 0.95, tp1: 0, tp2: -0.21 } });
  } catch (error) {
